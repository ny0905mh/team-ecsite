...
public interface TblCartMapper {
	 ...
	 * @return 更新件数
	 */
	@Update("UPDATE tbl_cart set product_count = #{productCount}, updated_at = CURRENT_TIMESTAMP WHERE user_id = #{userId} and product_id = #{productId} ")
	 ...
	 * @return 件数
	 */
	@Select("SELECT product_count FROM tbl_cart WHERE user_id = #{userId}")
	 ...
	 * @return 件数
	 */
	@Select("SELECT COALESCE((SELECT product_count FROM tbl_cart WHERE user_id = #{userId} AND product_id = #{productId}), 0)")
